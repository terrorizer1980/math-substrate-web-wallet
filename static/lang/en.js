module.exports = {
  lang: "English",
  webwallet_mobile_app: "APP",
  webwallet_your: "Your",
  webwallet_wallet: "Wallet",
  webwallet_home_des1: "Send, Request and Swap",
  webwallet_home_des2: "directly from your Blockchain Wallet.",
  webwallet_login: "Login via Math Wallet Browser Extension",
  webwallet_copy_address: "Copy Address",
  webwallet_qr_code: "QR Code",
  webwallet_estimated_value: "Estimated Value",
  webwallet_total: "Total",
  webwallet_refresh: "Refresh",
  webwallet_logout: "Logout",
  webwallet_transactions: "Transactions",
  webwallet_erc20_transactions: "Erc20 Token Txns",
  webwallet_to_address: "To Address",
  webwallet_to_address_pl: "Recipient Address",
  webwallet_cheap: "Cheap",
  webwallet_fast: "Fast",
  webwallet_simple: "Simple",
  webwallet_advanced: "Advanced",
  webwallet_memo_pl: "Maximum 30 characters (optional)",
  webwallet_fee: "Fee",
  webwallet_txHash: "TxHash",
  webwallet_type: "Type",
  webwallet_value: "Value",
  webwallet_status: "Status",
  webwallet_block_height: "Block Height",
  webwallet_tx_time: "Tx Time",
  transaction_information: "Transaction Information",
  webwallet_refresh_balance: 'Refresh Balance',
  available: "Available",
  delegate: "Delegated",
  undelegate: "Undelegating",
  transfer: "Transfer",
  transfer_amount: "Transfer Amount",
  memo: "Memo",
  noTransaction: "No transaction",
  noMathExtension: "Please connect Math Wallet first",
  from: "From",
  to: "To",
  transfer_account_null: "Please enter receive address",
  transfer_amount_null: "Please set transfer amount",
  copy_success: "Copied",
  copy_fail: "Please copy manually",
  transfer_success: "Transferred",
  transfer_fail: "Transfer failed",
  transfer_amount_min: "The transfer amount cannot be lower than ",
  back_transactions: "Back to Transaction Record",
  gas_limit: "Gas Limit",
  gas_used: "Gas Used",
  gas_price: 'Gas Price',
  mathExtensionLock: "Please unlock your Math Wallet",
  contract: "Contract",
  TRC20_transfer: "TRC20 Transfers",
  check_account: 'The current account is not the account selected by extension',
  gas_price_min: 'Gas Price cannot be lower than 1Gwei',
  gas_limit_min: 'Gas Limit cannot be lower than ',
  prompt: 'Prompt',
  input_data: 'Input Data',
  transfer_input: 'Please enter hexadecimal data (optional)',
  locked: 'Locked',
  submit_success: 'Submit success',
  gas_limit_null: 'Gas Limit is null',
  gas_Price_null: 'Gas Price is null',
  webwallet_harmony_amount_notenough: 'Have no enough available balance',
  node_not_available:'Current node is not available',
  webwallet_extrinsicID: 'Extrinsic ID',
  webwallet_module: 'Module',
  webwallet_signature: 'Signature',
  webwallet_address: 'Address',
  webwallet_transfers: 'Transfers',
  noTransfer: 'No transfer',
  Confirm:'Confirm',
  confirmed: 'Confirmed',
  unconfirmed: 'Unconfirmed',
  confirmations: 'Confirmations',
  transferring: 'Signing...',
  transfer_same_account: 'Cannot transfer tokens to the same account',
  ledgerLock: 'Please unlock your Ledger Wallet',
  ledger_login: 'Login via Ledger',
  add: 'Add',
  receive_tips:'This address can only be used to receive %w. Do not send other tokens to this address.',
  receive: 'Receive',
  noAdded: 'No tokens to add',
  token_add_successful: 'Successfully',
  token_add_fail: 'Failed'
}
