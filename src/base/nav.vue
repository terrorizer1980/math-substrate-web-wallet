<template>
  <nav class="math-nav clearfix">
    <div class="float-right">
      ...
      <div class="dropdown polka-node">
        <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" role="button">{{selectedChain}}<span class="tri"></span></a>
        <ul class="dropdown-menu">
          <li v-for="(node,index) in chainNameList" @click="changeNode(node, index)">{{node}}</li>
        </ul>
      </div>
      <div class="dropdown">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...mapActions(["setCurrentChain", "setSelectedChain"]),
      changeLang(lang) {
      ...
      changeNode(value, index){
        if (index == this.currentChain){
          return false;
        }
        this.setSelectedChain(index);
        this.setCurrentChain(index.toLowerCase());
        this.$emit('setCurrentNode',this.currentChain);
      }
    }
  }
</script>
